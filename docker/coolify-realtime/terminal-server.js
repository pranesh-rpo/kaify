...
const verifyClient = async (info, callback) => {
    ...
    try {
        ...
        const response = await axios.post(`http://kaify:8080/terminal/auth`, null, {
            headers: {
...
wss.on('connection', async (ws, req) => {
    ...
    const response = await axios.post(`http://kaify:8080/terminal/auth/ips`, null, {
        headers: {
            'Cookie': `${sessionCookieName}=${laravelSession}`,
            'X-XSRF-TOKEN': xsrfToken
        },
...
async function handleCommand(ws, command, userId) {
    ...
    //         Establishes an SSH connection to root@kaify with RequestTTY enabled
...
server.listen(6002, () => {
    console.log('Kaify realtime terminal server listening on port 6002. Let the hacking begin!');
});
