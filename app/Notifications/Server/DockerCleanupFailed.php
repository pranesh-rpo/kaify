...
class DockerCleanupFailed extends CustomEmailNotification
{
    ...
    public function toMail(): MailMessage
    {
        ...
        $mail->subject("Kaify: [ACTION REQUIRED] Docker cleanup job failed on {$this->server->name}");
    ...
    public function toDiscord(): DiscordMessage
    {
        return new DiscordMessage(
            title: ':cross_mark: Kaify: [ACTION REQUIRED] Docker cleanup job failed on '.$this->server->name,
    ...
    {
        return [
            'message' => "Kaify: [ACTION REQUIRED] Docker cleanup job failed on {$this->server->name}!\n\n{$this->message}",
        ];
    }
    ...
    public function toSlack(): SlackMessage
    {
        return new SlackMessage(
            title: 'Kaify: [ACTION REQUIRED] Docker cleanup job failed',
