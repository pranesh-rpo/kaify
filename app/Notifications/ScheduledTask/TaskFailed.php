...
class TaskFailed extends CustomEmailNotification
{
    ...
    public function toMail(): MailMessage
    {
        ...
        $mail->subject("Kaify: [ACTION REQUIRED] Scheduled task ({$this->task->name}) failed.");
    ...
    {
        $message = "Kaify: Scheduled task ({$this->task->name}) failed with output: {$this->output}";
        if ($this->url) {
            $buttons[] = [
                'text' => 'Open task in Kaify',
                'url' => (string) $this->url,
            ];
        }
