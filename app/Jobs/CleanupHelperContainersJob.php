...
class CleanupHelperContainersJob implements ShouldBeEncrypted, ShouldBeUnique, ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            $containers = instant_remote_process_with_timeout(['docker container ps --format \'{{json .}}\' | jq -s \'map(select(.Image | contains("'.config('constants.kaify.registry_url').'/kaify/kaify-helper")))\''], $this->server, false);
