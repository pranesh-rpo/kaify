                                    ...
                                    <template x-if="upgradeError">
                                        <div class="flex flex-col items-center gap-4">
                                            <p class="text-sm text-neutral-600 dark:text-neutral-400">
                                                Check the logs on the server at /data/kaify/source/upgrade*.
                                            </p>
                                ...
                                <div class="space-y-4">
                                    ...
                                    <p class="text-sm text-neutral-600 dark:text-neutral-400">
                                        ...
                                        <a class="font-medium underline dark:text-white hover:text-neutral-800 dark:hover:text-neutral-300"
                                            href="https://kaify.io/docs/upgrade" target="_blank">upgrade guide</a> or the
                                        logs on the server at /data/kaify/source/upgrade*.
                                    </p>
                                </div>
                            </template>
                        </div>
        ...
        Alpine.data('upgradeModal', (config) => ({
            ...
            simulateUpgrade() {
                ...
                const steps = [
                    ...
                    { step: 3, status: '[DEV] Pulling Kaify image...' },
            ...
            mapStepToUI(apiStep) {
                ...
                // UI: 1=prepare, 2=pull images, 3=pull kaify, 4=restart
            ...
            getReviveStatusMessage(elapsedMinutes, attempts) {
                if (elapsedMinutes === 0) {
                    return `Waiting for Kaify to come back online... (attempt ${attempts})`;
                } else if (elapsedMinutes < 2) {
                    return `Waiting for Kaify to come back online... (${elapsedMinutes} minute${elapsedMinutes !== 1 ? 's' : ''} elapsed)`;
                ...
                this.checkUpgradeStatusInterval = setInterval(async () => {
                    try {
                        ...
                        if (!this.serviceDown) {
                            ...
                            this.currentStep = 4;
                            this.currentStatus = 'Kaify is restarting with the new version...';
                            if (this.checkUpgradeStatusInterval) {
                                clearInterval(this.checkUpgradeStatusInterval);
                                this.checkUpgradeStatusInterval = null;
                            }
