...
it('has UpdateKaify action class', function () {
    expect(class_exists(UpdateKaify::class))->toBeTrue();
});
...
it('validates cache against running version before fallback', function () {
    ...
    Cache::shouldReceive('remember')
        ->andReturn(['kaify' => ['v1' => ['version' => '4.0.5']]]);

    config(['constants.kaify.version' => '4.0.10']);

    $action = new UpdateKaify;
...
it('uses validated cache when CDN fails and cache is newer', function () {
    ...
    Cache::shouldReceive('remember')
        ->andReturn(['kaify' => ['v1' => ['version' => '4.0.10']]]);

    config(['constants.kaify.version' => '4.0.5']);

    // Mock the update method to prevent actual update
    $action = Mockery::mock(UpdateKaify::class)->makePartial();
