...
function collectProxyDockerNetworksByServer(Server $server)
{
    ...
    $networks = instant_remote_process(['docker inspect --format="{{json .NetworkSettings.Networks }}" kaify-proxy'], $server, false);
...
function collectDockerNetworksByServer(Server $server)
{
    ...
    if ($server->isSwarm()) {
        if ($networks->count() === 0) {
            $networks = collect(['kaify-overlay']);
            $allNetworks = collect(['kaify-overlay']);
        }
    } else {
        if ($networks->count() === 0) {
            $networks = collect(['kaify']);
            $allNetworks = collect(['kaify']);
        }
    }
    ...
    if ($server->isSwarm()) {
        $commands = $networks->map(function ($network) {
            return [
                ...
                "docker network connect $network kaify-proxy >/dev/null 2>&1 || true",
                "echo 'Successfully connected kaify-proxy to $network network.'",
            ];
        });
    } else {
        $commands = $networks->map(function ($network) {
...
function extractCustomProxyCommands(Server $server, string $existing_config): array
{
    ...
    try {
        ...
        // Define default commands that Kaify generates
...
function generateDefaultProxyConfiguration(Server $server, array $custom_commands = [])
{
    ...
    if ($proxy_type === ProxyTypes::TRAEFIK->value) {
        ...
        if (isDev()) {
            ...
            $config['services']['traefik']['volumes'][] = '/var/lib/docker/volumes/kaify_dev_kaify_data/_data/proxy/:/traefik';
        } else {
...
function getExactTraefikVersionFromContainer(Server $server): ?string
{
    try {
        ...
        $versionCommand = "docker exec kaify-proxy traefik version 2>/dev/null | grep -oP 'Version:\s+\K\d+\.\d+\.\d+'";
        ...
        $labelCommand = "docker inspect kaify-proxy --format '{{index .Config.Labels \"org.opencontainers.image.version\"}}' 2>/dev/null";
