...
function applicationParser(Application $resource, int $pull_request_id = 0, ?int $preview_id = null, ?string $commit = null): Collection
{
    ...
    foreach ($services as $serviceName => $service) {
        ...
        // Add Kaify defined environments
        ...
        if ($magicEnvironments->count() > 0) {
            // Generate Kaify environment variables
    ...
    foreach ($services as $serviceName => $service) {
            ...
            foreach ($volumes as $index => $volume) {
                ...
                if ($type->value() === 'bind') {
                    if ($source->value() === '/var/run/docker.sock') {
                        ...
                        if (isDev()) {
                            if ((int) $resource->compose_parsing_version >= 4) {
                                $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/kaify_dev_kaify_data/_data/applications/'.$uuid);
                            } else {
                                $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/kaify_dev_kaify_data/_data/applications/'.$uuid);
                            }
                        }
            ...
            if ($key->value() === $parsedValue->value()) {
                ...
                if ($value->startsWith('$')) {
                    ...
                    if ($originalValue->value() === $value->value()) {
                        // This means the variable does not have a default value, so it needs to be created in Kaify
        ...
        if ($originalResource->environment_variables->where('key', 'KAIFY_BRANCH')->isEmpty()) {
            $kaifyEnvironments->put('KAIFY_BRANCH', "\"{$branch}\"");
        }

        // Add KAIFY_RESOURCE_UUID to environment
        if ($resource->environment_variables->where('key', 'KAIFY_RESOURCE_UUID')->isEmpty()) {
            $kaifyEnvironments->put('KAIFY_RESOURCE_UUID', "{$resource->uuid}");
        }

        // Add KAIFY_CONTAINER_NAME to environment
        if ($resource->environment_variables->where('key', 'KAIFY_CONTAINER_NAME')->isEmpty()) {
            $kaifyEnvironments->put('KAIFY_CONTAINER_NAME', "{$containerName}");
        }
        ...
        if ($resource->build_pack === 'dockercompose') {
            foreach ($domains as $forServiceName => $domain) {
                ...
                if (filled($parsedDomain)) {
                    ...
                    $kaifyUrl = Url::fromString($parsedDomain);
                    $kaifyScheme = $kaifyUrl->getScheme();
                    $kaifyFqdn = $kaifyUrl->getHost();
                    $kaifyUrl = $kaifyUrl->withScheme($kaifyScheme)->withHost($kaifyFqdn)->withPort(null);
                    $kaifyEnvironments->put('SERVICE_URL_'.str($forServiceName)->upper()->replace('-', '_')->replace('.', '_'), $kaifyUrl->__toString());
                    $kaifyEnvironments->put('SERVICE_FQDN_'.str($forServiceName)->upper()->replace('-', '_')->replace('.', '_'), $kaifyFqdn);
                    $resource->environment_variables()->updateOrCreate([
                        'resourceable_type' => Application::class,
                        'resourceable_id' => $resource->id,
                        'key' => 'SERVICE_URL_'.str($forServiceName)->upper()->replace('-', '_')->replace('.', '_'),
                    ], [
                        'value' => $kaifyUrl->__toString(),
                        'is_preview' => false,
                    ]);
                    $resource->environment_variables()->updateOrCreate([
                        'resourceable_type' => Application::class,
        ...
        $isDatabase = isDatabaseImage($image, $service);
        // Add KAIFY_FQDN & KAIFY_URL to environment
        if (! $isDatabase && $fqdns instanceof Collection && $fqdns->count() > 0) {
            $fqdnsWithoutPort = $fqdns->map(function ($fqdn) {
                return str($fqdn)->after('://')->before(':')->prepend(str($fqdn)->before('://')->append('://'));
            });
            $kaifyEnvironments->put('KAIFY_URL', $fqdnsWithoutPort->implode(','));

            $urls = $fqdns->map(function ($fqdn) {
                return str($fqdn)->replace('http://', '')->replace('https://', '')->before(':');
            });
            $kaifyEnvironments->put('KAIFY_FQDN', $urls->implode(','));
        }
        add_kaify_default_environment_variables($resource, $kaifyEnvironments, $resource->environment_variables);
        ...
        if ($environment->count() > 0 || $kaifyEnvironments->count() > 0) {
            $payload['environment'] = $environment->merge($kaifyEnvironments)->merge($serviceNameEnvironments);
        }
        if ($logging) {
        ...
        // Auto-inject .env file so Kaify environment variables are available inside containers
    ...
    // Parse the original compose again to create a clean version without Kaify additions
    try {
...
function serviceParser(Service $resource): Collection
{
        ...
        foreach ($environment as $key => $value) {
            ...
            if ($key->startsWith('SERVICE_FQDN_') || $key->startsWith('SERVICE_URL_')) {
                ...
                if ($isServiceApplication && is_null($savedService->fqdn)) {
                    ...
                    if ((int) $resource->compose_parsing_version >= 5 && version_compare(config('constants.kaify.version'), '1.0.0', '>=')) {
    ...
    foreach ($services as $serviceName => $service) {
            ...
            foreach ($volumes as $index => $volume) {
                ...
                if ($type->value() === 'bind') {
                    if ($source->value() === '/var/run/docker.sock') {
                        ...
                        if (isDev()) {
                            if ((int) $resource->compose_parsing_version >= 4) {
                                $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/kaify_dev_kaify_data/_data/services/'.$uuid);
                            } else {
                                $source = $source->replace($mainDirectory, '/var/lib/docker/volumes/kaify_dev_kaify_data/_data/applications/'.$uuid);
                            }
                        }
        ...
        if (! $isDatabase && $fqdns instanceof Collection && $fqdns->count() > 0) {
            ...
            $kaifyEnvironments->put('KAIFY_FQDN', $fqdnsWithoutPort->implode(','));
            $urls = $fqdns->map(function ($fqdn): Stringable {
                return str($fqdn)->after('://')->before(':')->prepend(str($fqdn)->before('://')->append('://'));
            });
            $kaifyEnvironments->put('KAIFY_URL', $urls->implode(','));
        }
        add_kaify_default_environment_variables($resource, $kaifyEnvironments, $resource->environment_variables);
        if ($environment->count() > 0) {
            $environment = $environment->filter(function ($value, $key) {
