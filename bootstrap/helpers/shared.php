...
{
    try {
        $response = Http::get(config('constants.kaify.versions_url'));
        $versions = $response->json();

        return data_get($versions, 'kaify.sentinel.version');
    } catch (\Throwable) {
        return '0.0.0';
    }
}
function get_latest_version_of_kaify(): string
{
    try {
        $versions = get_versions_data();

        return data_get($versions, 'kaify.v1.version', '0.0.0');
    } catch (\Throwable $e) {
...
function generateSSHKey(string $type = 'rsa')
{
    if ($type === 'rsa') {
        ...
        return [
            ...
            'public' => $key->getPublicKey()->toString('OpenSSH', ['comment' => 'kaify-generated-ssh-key']),
        ];
    } elseif ($type === 'ed25519') {
...
{
    return ! config('constants.kaify.self_hosted');
}
...
function generateFqdn(Server $server, string $random, bool $forceHttps = false, int $parserVersion = 5): string
{
    ...
    if ($parserVersion >= 5 && version_compare(config('constants.kaify.version'), '1.0.0', '>=')) {
...
function parseDockerComposeFile(Service|Application $resource, bool $isNew = false, int $pull_request_id = 0, ?int $preview_id = null)
{
    if ($resource->getMorphClass() === \App\Models\Service::class) {
        if ($resource->docker_compose_raw) {
            ...
            $envs_from_kaify = $resource->environment_variables()->get();
            $services = collect($services)->map(function ($service, $serviceName) use ($resource, $envs_from_kaify) {
                $serviceVariables = collect(data_get($service, 'environment', []));
                ...
                $parsedServiceVariables->put('KAIFY_RESOURCE_UUID', "{$resource->uuid}");
                $parsedServiceVariables->put('KAIFY_CONTAINER_NAME', "$serviceName-{$resource->uuid}");

                // TODO: move this in a shared function
                if (! $parsedServiceVariables->has('KAIFY_APP_NAME')) {
                    $parsedServiceVariables->put('KAIFY_APP_NAME', "\"{$resource->name}\"");
                }
                if (! $parsedServiceVariables->has('KAIFY_SERVER_IP')) {
                    $parsedServiceVariables->put('KAIFY_SERVER_IP', "\"{$resource->destination->server->ip}\"");
                }
                if (! $parsedServiceVariables->has('KAIFY_ENVIRONMENT_NAME')) {
                    $parsedServiceVariables->put('KAIFY_ENVIRONMENT_NAME', "\"{$resource->environment->name}\"");
                }
                if (! $parsedServiceVariables->has('KAIFY_PROJECT_NAME')) {
                    $parsedServiceVariables->put('KAIFY_PROJECT_NAME', "\"{$resource->project()->name}\"");
                }

                $parsedServiceVariables = $parsedServiceVariables->map(function ($value, $key) use ($envs_from_kaify) {
                    if (! str($value)->startsWith('$')) {
                        $found_env = $envs_from_kaify->where('key', $key)->first();
                        if ($found_env) {
...
function generate_fluentd_configuration(): array
{
    return [
        ...
        'options' => [
            ...
            'env' => 'KAIFY_APP_NAME,KAIFY_PROJECT_NAME,KAIFY_SERVER_IP,KAIFY_ENVIRONMENT_NAME',
        ],
    ];
}
 ...
 * - KAIFY_PROJECT_NAME
 * - KAIFY_SERVER_IP
 * - KAIFY_ENVIRONMENT_NAME
 *
 *  Theses variables are added in place to the $where_to_add array.
 */
function add_kaify_default_environment_variables(StandaloneRedis|StandalonePostgresql|StandaloneMongodb|StandaloneMysql|StandaloneMariadb|StandaloneKeydb|StandaloneDragonfly|StandaloneClickhouse|Application|Service $resource, Collection &$where_to_add, ?Collection $where_to_check = null)
{
    // Currently disabled
    return;
    ...
    if ($where_to_check != null && $where_to_check->where('key', 'KAIFY_APP_NAME')->isEmpty()) {
        if ($isAssociativeArray) {
            $where_to_add->put('KAIFY_APP_NAME', "\"{$resource->name}\"");
        } else {
            $where_to_add->push("KAIFY_APP_NAME=\"{$resource->name}\"");
        }
    }
    if ($where_to_check != null && $where_to_check->where('key', 'KAIFY_SERVER_IP')->isEmpty()) {
        if ($isAssociativeArray) {
            $where_to_add->put('KAIFY_SERVER_IP', "\"{$ip}\"");
        } else {
            $where_to_add->push("KAIFY_SERVER_IP=\"{$ip}\"");
        }
    }
    if ($where_to_check != null && $where_to_check->where('key', 'KAIFY_ENVIRONMENT_NAME')->isEmpty()) {
        if ($isAssociativeArray) {
            $where_to_add->put('KAIFY_ENVIRONMENT_NAME', "\"{$resource->environment->name}\"");
        } else {
            $where_to_add->push("KAIFY_ENVIRONMENT_NAME=\"{$resource->environment->name}\"");
        }
    }
    if ($where_to_check != null && $where_to_check->where('key', 'KAIFY_PROJECT_NAME')->isEmpty()) {
        if ($isAssociativeArray) {
            $where_to_add->put('KAIFY_PROJECT_NAME', "\"{$resource->project()->name}\"");
        } else {
            $where_to_add->push("KAIFY_PROJECT_NAME=\"{$resource->project()->name}\"");
        }
    }
}
...
function getHelperVersion(): string
{
    ...
    return config('constants.kaify.helper_version');
}
...
function loadConfigFromGit(string $repository, string $branch, string $base_directory, int $server_id, int $team_id)
{
    ...
    $fileList = collect([".$workdir/kaify.json"]);
    $commands = collect([
        "rm -rf /tmp/{$uuid}",
        "mkdir -p /tmp/{$uuid}",
        "cd /tmp/{$uuid}",
