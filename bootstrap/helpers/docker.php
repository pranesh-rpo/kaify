...
function getCurrentApplicationContainerStatus(Server $server, int $id, ?int $pullRequestId = null, ?bool $includePullrequests = false): Collection
{
    $containers = collect([]);
    if (! $server->isSwarm()) {
        $containers = instant_remote_process(["docker ps -a --filter='label=kaify.applicationId={$id}' --format '{{json .}}' "], $server);
        $containers = format_docker_command_output_to_json($containers);

        $containers = $containers->map(function ($container) use ($pullRequestId, $includePullrequests) {
            $labels = data_get($container, 'Labels');
            $containerName = data_get($container, 'Names');
            $hasPrLabel = str($labels)->contains('kaify.pullRequestId=');
            $prLabelValue = null;

            if ($hasPrLabel) {
                preg_match('/kaify\.pullRequestId=(\d+)/', $labels, $matches);
            ...
            if ($pullRequestId !== null && $pullRequestId !== 0 && str($labels)->contains("kaify.pullRequestId={$pullRequestId}")) {
                return $container;
            }
...
function getCurrentServiceContainerStatus(Server $server, int $id): Collection
{
    $containers = collect([]);
    if (! $server->isSwarm()) {
        $containers = instant_remote_process(["docker ps -a --filter='label=kaify.serviceId={$id}' --format '{{json .}}' "], $server);
        $containers = format_docker_command_output_to_json($containers);
...
{
    $labels = collect([]);
    $labels->push('kaify.managed=true');
    $labels->push('kaify.type=database');
    $labels->push('kaify.databaseId='.$database->id);
    $labels->push('kaify.resourceName='.Str::slug($database->name));
    $labels->push('kaify.serviceName='.Str::slug($database->name));
    $labels->push('kaify.projectName='.Str::slug($database->project()->name));
    $labels->push('kaify.environmentName='.Str::slug($database->environment->name));
    $labels->push('kaify.database.subType='.$database->type());
...
function defaultLabels($id, $name, string $projectName, string $resourceName, string $environment, $pull_request_id = 0, string $type = 'application', $subType = null, $subId = null, $subName = null)
{
    $labels = collect([]);
    $labels->push('kaify.managed=true');
    $labels->push('kaify.version='.config('constants.kaify.version'));
    $labels->push('kaify.'.$type.'Id='.$id);
    $labels->push("kaify.type=$type");
    $labels->push('kaify.name='.$name);
    $labels->push('kaify.resourceName='.Str::slug($resourceName));
    $labels->push('kaify.projectName='.Str::slug($projectName));
    $labels->push('kaify.serviceName='.Str::slug($subName ?? $resourceName));
    $labels->push('kaify.environmentName='.Str::slug($environment));

    $labels->push('kaify.pullRequestId='.$pull_request_id);
    if ($type === 'service') {
        $subId && $labels->push('kaify.service.subId='.$subId);
        $subType && $labels->push('kaify.service.subType='.$subType);
        $subName && $labels->push('kaify.service.subName='.Str::slug($subName));
    }
...
function fqdnLabelsForTraefik(string $uuid, Collection $domains, bool $is_force_https_enabled = false, $onlyPort = null, ?Collection $serviceLabels = null, ?bool $is_gzip_enabled = true, ?bool $is_stripprefix_enabled = true, ?string $service_name = null, bool $generate_unique_uuid = false, ?string $image = null, string $redirect_direction = 'both', bool $is_http_basic_auth_enabled = false, ?string $http_basic_auth_username = null, ?string $http_basic_auth_password = null)
{
    ...
    if ($serviceLabels) {
        $middlewares_from_labels = $serviceLabels->map(function ($item) {
            ...
            if (preg_match('/kaify\.traefik\.middlewares=(.*)/', $item, $matches)) {
...
/**
 * Remove Kaify's custom Docker Compose fields from parsed YAML array
 *
 * Kaify extends Docker Compose with custom fields that are processed during
 ...
 */
function stripKaifyCustomFields(array $yamlCompose): array
{
...
function validateComposeFile(string $compose, int $server_id): string|Throwable
{
    ...
    try {
        ...
        // Remove Kaify's custom fields before Docker validation
        $yaml_compose = stripKaifyCustomFields($yaml_compose);
