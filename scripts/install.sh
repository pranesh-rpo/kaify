...
CDN="https://raw.githubusercontent.com/pranesh-rpo/kaify/main"
DATE=$(date +"%Y%m%d-%H%M%S")

OS_TYPE=$(grep -w "ID" /etc/os-release | cut -d "=" -f 2 | tr -d '"')
ENV_FILE="/data/kaify/source/.env"
DOCKER_VERSION="27.0"
# TODO: Ask for a user
CURRENT_USER=$USER
...
echo ""
echo "=========================================="
echo "   Kaify Installation - ${DATE}"
echo "=========================================="
echo ""
echo "Welcome to Kaify Installer!"
echo "This script will install everything for you. Sit back and relax."
echo "Source code: https://github.com/pranesh-rpo/kaify/blob/main/scripts/install.sh"
...
if [ -f "/data/kaify/source/.env" ] && [ "$DOCKER_POOL_BASE_PROVIDED" = false ] && [ "$DOCKER_POOL_SIZE_PROVIDED" = false ]; then
    ENV_DOCKER_ADDRESS_POOL_BASE=$(grep -E "^DOCKER_ADDRESS_POOL_BASE=" /data/kaify/source/.env | cut -d '=' -f2 || true)
    ENV_DOCKER_ADDRESS_POOL_SIZE=$(grep -E "^DOCKER_ADDRESS_POOL_SIZE=" /data/kaify/source/.env | cut -d '=' -f2 || true)
...
mkdir -p /data/kaify/{source,ssh,applications,databases,backups,services,proxy,sentinel}
mkdir -p /data/kaify/ssh/{keys,mux}
mkdir -p /data/kaify/proxy/dynamic

chown -R 9999:root /data/kaify
chmod -R 700 /data/kaify

INSTALLATION_LOG_WITH_DATE="/data/kaify/source/installation-${DATE}.log"
...
if [ "$SSH_DETECTED" = "false" ]; then
    ...
    *)
        ...
        echo -e "Please make sure it is installed and running, otherwise Kaify cannot connect to the host system. \n"
        echo "###############################################################################"
        exit 1
        ;;
    esac
...
if [ "$SSH_PERMIT_ROOT_LOGIN" = "yes" ] || [ "$SSH_PERMIT_ROOT_LOGIN" = "without-password" ] || [ "$SSH_PERMIT_ROOT_LOGIN" = "prohibit-password" ]; then
    ...
    echo "   If you have problems with SSH, please read this: https://kaify.io/docs/knowledge-base/server/openssh"
fi
...
if [ -x "$(command -v snap)" ]; then
    ...
    if [ "$SNAP_DOCKER_INSTALLED" = "true" ]; then
        ...
        echo "   Please note that Kaify does not support Docker installed via snap."
...
# Create kaify configuration with or without address pools based on whether they were explicitly provided
...
EOL
    ...
    if [ -f /etc/docker/daemon.json ] && jq -e '.["log-driver"] == "json-file" and .["log-opts"]["max-size"] == "10m" and .["log-opts"]["max-file"] == "3"' /etc/docker/daemon.json >/dev/null 2>&1; then
        ...
        cat >/etc/docker/daemon.json.kaify <<EOL
{
  "log-driver": "json-file",
  "log-opts": {
    "max-size": "10m",
...
curl -fsSL -L $CDN/docker-compose.yml -o /data/kaify/source/docker-compose.yml &
PID1=$!
curl -fsSL -L $CDN/docker-compose.prod.yml -o /data/kaify/source/docker-compose.prod.yml &
PID2=$!
...
if [ -f "$ENV_FILE" ]; then
    ...
    awk -F '=' '!seen[$1]++' "$ENV_FILE" "/data/kaify/source/.env.production" > "$ENV_FILE.tmp" && mv "$ENV_FILE.tmp" "$ENV_FILE"
    ...
    cp "/data/kaify/source/.env.production" "$ENV_FILE"
fi
...
set +e
IS_KAIFY_VOLUME_EXISTS=$(docker volume ls | grep kaify-db | wc -l)
set -e

if [ "$IS_KAIFY_VOLUME_EXISTS" -eq 0 ]; then
    echo " - Generating SSH key."
    test -f /data/kaify/ssh/keys/id.$CURRENT_USER@host.docker.internal && rm -f /data/kaify/ssh/keys/id.$CURRENT_USER@host.docker.internal
    test -f /data/kaify/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub && rm -f /data/kaify/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub
    ssh-keygen -t ed25519 -a 100 -f /data/kaify/ssh/keys/id.$CURRENT_USER@host.docker.internal -q -N "" -C kaify
    chown 9999 /data/kaify/ssh/keys/id.$CURRENT_USER@host.docker.internal
    sed -i "/kaify/d" ~/.ssh/authorized_keys
    cat /data/kaify/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub >>~/.ssh/authorized_keys
    rm -f /data/kaify/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub
fi
...
log_section "Step 9/9: Installing Kaify"
echo "9/9 Installing Kaify ($LATEST_VERSION)..."
echo -e " - It could take a while based on your server's performance, network speed, stars, etc."
echo -e " - Please wait."
getAJoke

if [[ $- == *x* ]]; then
    bash -x /data/kaify/source/upgrade.sh "${LATEST_VERSION:-latest}" "${LATEST_HELPER_VERSION:-latest}" "${REGISTRY_URL:-ghcr.io}" "true"
else
    bash /data/kaify/source/upgrade.sh "${LATEST_VERSION:-latest}" "${LATEST_HELPER_VERSION:-latest}" "${REGISTRY_URL:-ghcr.io}" "true"
fi
echo " - Kaify installed successfully."
echo " - Waiting for Kaify to be ready..."

# Wait for upgrade.sh background process to complete
# upgrade.sh writes status to /data/kaify/source/.upgrade-status
# Status file format: step|message|timestamp
# Step 6 = "Upgrade complete", file deleted 10 seconds after
UPGRADE_STATUS_FILE="/data/kaify/source/.upgrade-status"
MAX_WAIT=180
WAITED=0
...
while [ $WAITED -lt $MAX_WAIT ]; do
    if [ -f "$UPGRADE_STATUS_FILE" ]; then
        ...
        if [ "$STATUS" = "6" ]; then
            ...
            echo " - Please check the upgrade logs: /data/kaify/source/upgrade-*.log"
            exit 1
        else
...
echo " - Verifying Kaify is healthy..."
HEALTH_WAIT=60
HEALTH_WAITED=0
while [ $HEALTH_WAITED -lt $HEALTH_WAIT ]; do
    HEALTH=$(docker inspect --format='{{.State.Health.Status}}' kaify 2>/dev/null || echo "unknown")
    if [ "$HEALTH" = "healthy" ]; then
        log "Kaify container is healthy"
        echo " - Kaify is ready!"
        break
    fi
    sleep 2
...
if [ "$HEALTH" != "healthy" ]; then
    echo " - ERROR: Kaify container is not healthy after ${HEALTH_WAIT}s. Status: $HEALTH"
    echo " - Please check: docker logs kaify"
    exit 1
fi
echo -e "\033[0;35m
   ____                            _         _       _   _                 _
...
if [ -n "$IPV4_PUBLIC_IP" ]; then
    echo -e "You can access Kaify through your Public IPV4: http://$IPV4_PUBLIC_IP:8000"
fi
if [ -n "$IPV6_PUBLIC_IP" ]; then
    echo -e "You can access Kaify through your Public IPv6: http://[$IPV6_PUBLIC_IP]:8000"
fi
...
echo -e "\nWARNING: It is highly recommended to backup your Environment variables file (/data/kaify/source/.env) to a safe location, outside of this server (e.g. into a Password Manager).\n"

log_section "Installation Complete"
log "Kaify installation completed successfully"
