...
STATUS_FILE="/data/kaify/source/.upgrade-status"

DATE=$(date +%Y-%m-%d-%H-%M-%S)
LOGFILE="/data/kaify/source/upgrade-${DATE}.log"
...
echo ""
echo "=========================================="
echo "   Kaify Upgrade - ${DATE}"
echo "=========================================="
echo ""

# Initialize log file with header
echo "============================================================" >>"$LOGFILE"
echo "Kaify Upgrade Log" >>"$LOGFILE"
...
COMPOSE_FILES="-f /data/kaify/source/docker-compose.yml -f /data/kaify/source/docker-compose.prod.yml"

# Check if custom compose file exists
if [ -f /data/kaify/source/docker-compose.custom.yml ]; then
    COMPOSE_FILES="$COMPOSE_FILES -f /data/kaify/source/docker-compose.custom.yml"
...
# Make sure kaify network exists
# It is created when starting Kaify with docker compose
log "Checking Docker network 'kaify'..."
if ! docker network inspect kaify >/dev/null 2>&1; then
    log "Network 'kaify' does not exist, creating..."
    if ! docker network create --attachable --ipv6 kaify 2>/dev/null; then
        log "Failed to create network with IPv6, trying without IPv6..."
        docker network create --attachable kaify 2>/dev/null
        log "Network 'kaify' created without IPv6"
    else
        log "Network 'kaify' created with IPv6 support"
    fi
else
    log "Network 'kaify' already exists"
fi
...
echo "     This step will restart all Kaify containers."
echo "     Check the log file for details: ${LOGFILE}"

# From this point forward, we need to ensure the script continues even if
# the SSH connection is lost (which happens when kaify container stops)
...
nohup bash -c "
    ...
    for container in kaify kaify-db kaify-redis kaify-realtime; do
    ...
    log 'Kaify upgrade completed successfully'
...
echo ""
echo "=========================================="
echo "   Kaify upgrade to ${LATEST_IMAGE} in progress"
echo "=========================================="
echo ""
echo "   The upgrade will continue in the background."
echo "   Kaify will be available again shortly."
